...
MassUpdateAccountsBatch (Database.Batchable)
   ...
   ▼
Bulk_Job_Settled__e (Platform Event)
...
├── objects/
│   └── Bulk_Job_Settled__e/               # Platform Event definition and fields
...
A dedicated, minimal Permission Set named `Callout Permission`, granting only External Credential Principal Access (`Claude_Routine_Credential` / `AnthropicPrincipal`), assigned directly to the org's Automated Process User — no point-and-click path exists for this step, it must be done via Apex:
```apex
...
```
Deploy order:
`objects/Bulk_Job_Settled__e` (Platform Event + fields)
`classes/` (Batch, REST endpoint, Queueable)
`triggers/BulkJobSettledTrigger`
Create the `Callout Permission` Permission Set and assign it to the Automated Process User (Execute Anonymous — see Setup section above)
`externalServiceRegistrations/` (registers the REST endpoint for MCP tool discovery)
Attach the resulting operation to your MCP Server definition in Setup
Testing
...
License
(Add your chosen license here — see repo settings for guidance on what reuse terms you want to apply.)
